...
export default function InputBox({ icon, label, placeholder, buttonText, value, onClick }: InputBoxProps) {
    ...
    return (
        <div className="w-full flex flex-col">
            <div onClick={() => setDropdownOpen(!dropdownOpen)} className="p-2 flex gap-2">
            ...
            <div className="w-full flex gap-2">
                ...
                <button onClick={onClick} className="w-1/3 p-2 bg-[#0E7490] text-white rounded-full hover:bg-[#0891B2] transition-transform duration-200 ease-in-out hover:scale-105 hover:cursor-pointer">{buttonText}</button>
            </div>
            )}
        </div>
    )
