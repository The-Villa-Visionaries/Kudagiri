...
1. **Hotels and rooms:** FastAPI startup, the master API router, SQLite models,
   catalogue queries, validation, and tests. This is Part 1.
2. **Authentication:** `api/auth.py`, user model/schema, password hashing and
   token helpers, signup/login service, and authentication tests.
3. **Bookings and tickets:** `api/bookings.py`, booking/ticket models and schemas,
   ...
   double-booking protection, plus tests. Register both later routers in `api/router.py`.
...
will need a migration in future; startup only creates missing tables.
